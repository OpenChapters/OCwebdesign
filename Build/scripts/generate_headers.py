...
CHAPTERS = {
    ...
    "FOURIE": {
        "title": "Fourier Transforms and Series",
        "bg": ("#ede7f6", "#e1f5fe"),
        "accents": ["#4527a0", "#00695c", "#5e35b1", "#00838f", "#3949ab"],
        "formulas": [
            r"$f(x)=a_0+\sum_n a_n\cos\frac{n\pi x}{L}+\sum_n b_n\sin\frac{n\pi x}{L}$",
            r"$f(x)=\sum_n A_n\,e^{\pi i n x/L}$",
            r"$A_n=\frac{1}{2L}\int_{-L}^{+L}f(x)\,e^{-\pi i n x/L}\,dx$",
            r"$\int\delta(\mathbf{r}-\mathbf{a})\,d\mathbf{r}=1$",
            r"$\delta(x)=\lim_{a\to\infty}\frac{a}{\sqrt{\pi}}\,e^{-a^2x^2}$",
            r"$f(\mathbf{r})\;\leftrightarrow\;F(\mathbf{k})$",
            r"$e^{2\pi i\,\mathbf{k}\cdot\mathbf{r}}$",
            r"$\Pi(x)=H(x+\frac{1}{2})-H(x-\frac{1}{2})$",
        ],
        "big_symbols": [r"$\delta$", r"$\Pi$", r"$\int$", r"$\sum$"],
    },
    "SCATTR": {
        "title": "Particle Scattering",
        "bg": ("#e1f5fe", "#e0f2f1"),
        "accents": ["#01579b", "#00695c", "#0277bd", "#00838f", "#1565c0"],
        "formulas": [
            r"$\lambda = \frac{h}{p}$",
            r"$E = h\nu = \frac{hc}{\lambda}$",
            r"$I(\alpha)=I_0\,\frac{r_e^2}{r^2}\sin^2\alpha$",
            r"$I_P=I_0\,\frac{r_e^2}{r^2}\,\frac{1+\cos^2 2\theta}{2}$",
            r"$f(s)=\sum_i a_i\,e^{-b_i s^2}$",
            r"$\bar{I}(k)=1+\frac{1}{N}\sum_{m\ne n}\frac{\sin(k r_{mn})}{k r_{mn}}$",
            r"$\varphi(\mathbf{r})=\frac{B}{r}\,e^{2\pi i k r}$",
            r"$\mathbf{E}=\mathbf{E}_0\,e^{2\pi i(\mathbf{k}\cdot\mathbf{r}-\nu t)}$",
        ],
        "big_symbols": [r"$\lambda$", r"$f(s)$", r"$\theta$", r"$\mathbf{k}$"],
    },
    "DIFGEO": {
        "title": "The Geometry of Diffraction Processes",
        "bg": ("#e0f7fa", "#e8eaf6"),
        "accents": ["#006064", "#283593", "#00838f", "#1a237e", "#0097a7"],
        "formulas": [
            r"$n\lambda = 2 d_{hkl}\sin\theta$",
            r"$d_{hkl}=1/|\mathbf{g}_{hkl}|$",
            r"$\mathbf{g}_{hkl}=h\mathbf{a}^*+k\mathbf{b}^*+l\mathbf{c}^*$",
            r"$\Delta\mathbf{k}=\mathbf{g}_{hkl}$",
            r"$|\mathbf{k}|=1/\lambda$",
            r"$s=\frac{\sin\theta}{\lambda}$",
            r"$\mathbf{a}_i\cdot\mathbf{a}_j^*=\delta_{ij}$",
        ],
        "big_symbols": [r"$\theta$", r"$\mathbf{g}$", r"$d_{hkl}$", r"$\lambda$"],
    },
    "ORISAM": {
        "title": "Orientation Sampling and Averaging",
        "bg": ("#e0f2f1", "#ede7f6"),
        "accents": ["#00695c", "#4527a0", "#00838f", "#5e35b1", "#0097a7"],
        "formulas": [
            r"$g(\theta;\mu,\kappa)=\frac{1}{2\pi I_0(\kappa)}\,e^{\kappa\cos(\theta-\mu)}$",
            r"$f(\mathbf{x};\mu,\kappa)\propto e^{\kappa\,\mu\cdot\mathbf{x}}$",
            r"$f_W(q;\mu,\kappa)\propto e^{\kappa(\mu\cdot q)^2}$",
            r"$E_s=\sum_{i\ne j}\|q_i-q_j\|^{-s}$",
            r"$\bar{C}=\frac{1}{n}\sum_i\cos\theta_i,\;\bar{S}=\frac{1}{n}\sum_i\sin\theta_i$",
            r"$q_m=q_1^{*}\,q_2$",
            r"$\bar{\mathbf{x}}=\frac{1}{n}\sum_i\mathbf{x}_i$",
        ],
