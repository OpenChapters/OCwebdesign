...
12. [API Reference (Swagger UI)](#api-reference-swagger-ui)
...
The server exposes its OpenAPI 3 schema and two interactive doc views generated by drf-spectacular. They cover every route the frontend uses, including the admin-only endpoints under `/api/admin/`.

| Path | What it is |
|---|---|
| `/api/schema/` | Raw OpenAPI 3 YAML — feed this into Postman, openapi-generator, or any client-codegen tool |
| `/api/schema/swagger-ui/` | Swagger UI — try-it-out console; auth via the **Authorize** button after `POST /api/auth/login/` |
| `/api/schema/redoc/` | ReDoc — clean, read-only docs (better for browsing than trying) |

The pages are public, but most endpoints require a JWT. To call admin endpoints from Swagger UI:

1. Open `/api/schema/swagger-ui/`.
2. Try `POST /api/auth/login/` with your admin credentials. Copy the `access` token from the response.
3. Click the green **Authorize** button at the top right. Paste `Bearer <access-token>` (include the literal word `Bearer` and a space).
4. Every subsequent **Try it out** request now sends the JWT. Useful admin operations to start with:
   - `GET /api/admin/examples/?status=pending` — current review queue
   - `POST /api/admin/examples/{id}/approve/` — approve an example
   - `POST /api/admin/examples/{id}/reject/` with `{"rejection_reason": "..."}` — reject one
   - `GET /api/admin/builds/` — recent book builds with status
   - `GET /api/health/` — sanity check

Access tokens expire after 30 minutes. If a call starts returning 401, re-run the login and re-authorize.

A handful of internal content-serving routes (figure files, HTML mirrors, signed PDF downloads, the chapter catalog CSV) are intentionally hidden from the schema — they don't belong in an API contract. They still work over plain HTTP if you need them.
