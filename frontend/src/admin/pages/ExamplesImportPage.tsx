...
type DefaultStatus = 'pending' | 'published';
...
export default function ExamplesImportPage() {
  ...
  async function handleValidate() {
    ...
    setCommitted(false);
    try {
      const r = await examplesApi.adminImportDryRun(file);
      setReport(r);
  ...
  async function handleCommit() {
    ...
    setLoading(true);
    try {
      const r = await examplesApi.adminImportCommit(file, defaultStatus);
      setReport(r);
      setCommitted(true);
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-5 mb-4 space-y-4">
        ...
        <div>
          ...
          <div className="flex gap-3">
            {(['pending', 'published'] as DefaultStatus[]).map((s) => (
              <label key={s} className="flex items-center gap-1.5 text-sm">
                ...
                />
                {s === 'pending' ? 'Pending review' : 'Published'}
              </label>
            ))}
          </div>
          <p className="text-xs text-gray-500 mt-1">
