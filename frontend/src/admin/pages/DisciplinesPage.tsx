  ...
  return (
    <div className="p-8 max-w-5xl">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-50">Disciplines</h1>
        <button
          onClick={openCreate}
      ...
      {showForm && (
        <div className="mb-6 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-5">
          ...
          {error && (
            <div className="mb-4 text-sm text-red-700 dark:text-red-300 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900 rounded-lg px-3 py-2">
              {error}
            </div>
          )}
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">GitHub Repo</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Source Path</label>
              <input
                type="text"
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  value={form.color_primary}
                  onChange={(e) => setForm((f) => ({ ...f, color_primary: e.target.value }))}
                  className="w-24 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Display Order</label>
              <input
                type="number"
                value={form.order}
                onChange={(e) => setForm((f) => ({ ...f, order: parseInt(e.target.value) || 0 }))}
                className="w-24 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
            <div className="flex items-center gap-2 col-span-2">
              ...
              />
              <label htmlFor="disc-published" className="text-sm text-gray-700 dark:text-gray-200">Published</label>
            </div>
          </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 text-left">
                <th className="px-4 py-2 font-medium text-gray-500 dark:text-gray-400 w-8">Order</th>
                <th className="px-4 py-2 font-medium text-gray-500 dark:text-gray-400">Color</th>
                ...
                <th className="px-4 py-2 font-medium text-gray-500 dark:text-gray-400">Repo</th>
                <th className="px-4 py-2 font-medium text-gray-500 dark:text-gray-400">Chapters</th>
            ...
            <tbody>
              {disciplines.length === 0 && (
                <tr><td colSpan={8} className="px-4 py-8 text-center text-gray-400 dark:text-gray-500">No disciplines found.</td></tr>
              )}
              {disciplines.map((disc) => (
                <tr key={disc.id} className="border-b border-gray-100 dark:border-gray-700 last:border-0 hover:bg-gray-50 dark:hover:bg-gray-700">
                  <td className="px-4 py-2 text-gray-500 dark:text-gray-400">{disc.order}</td>
                  <td className="px-4 py-2">
                    <span
                      className="inline-block w-5 h-5 rounded border border-gray-200 dark:border-gray-700"
                      style={{ backgroundColor: disc.color_primary }}
                      title={disc.color_primary}
                    />
                  </td>
                  <td className="px-4 py-2 font-medium text-gray-900 dark:text-gray-50">{disc.name}</td>
                  <td className="px-4 py-2 font-mono text-xs text-gray-600 dark:text-gray-300">{disc.slug}</td>
                  <td className="px-4 py-2 font-mono text-xs text-gray-600 dark:text-gray-300">{disc.github_repo}</td>
                  <td className="px-4 py-2 text-gray-600 dark:text-gray-300">{disc.chapter_count}</td>
                  <td className="px-4 py-2">
                    {disc.published ? (
